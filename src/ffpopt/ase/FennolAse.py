...
from fennol.models.preprocessing import convert_to_jax
...
class FENNIXCalculator(ase.calculators.calculator.Calculator):
    ...
    def calculate(
        ...
        if self.charge is None:
            total_charge = int(round(self.atoms.get_initial_charges().sum()))
            inputs["total_charge"] = total_charge
        else:
            total_charge = int(round(self.charge))
            inputs["total_charge"] = total_charge
        ...
        #print("charge=%8.3f energy=%14.6f"%(total_charge,results["energy"]))
