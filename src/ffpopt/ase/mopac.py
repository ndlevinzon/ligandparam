...
def generate_temp_filename() -> str:
    """Generates a probable unique temporary filename string."""
    import random
    import string
...
class MOPAC(FileIOCalculator):
    ...
    def write_input(self, atoms, properties=None, system_changes=None):
        ...
        # Write charge:
        #print(f"p.charge {p.charge} {atoms.get_initial_charges().sum()}")
        #print(p)
        if p.charge is None:
    ...
    def read_atoms_from_file(self, lines):
        ...
        if self.parameters.charge is not None:
            netq = self.parameters.charge
        else:
            netq = 0
        nat = len(symbols)
        charges = [netq / nat]*nat
    ...
    def read_results(self):
        ...
        for i, line in enumerate(lines):
            if total_energy_regex.match(line):
                ...
                for line in lines[i + 3:i + 3 + 3 * len(self.atoms)]:
                    # Extract using character indices
                    col6_str = line[36:49]
                    col7_str = line[50:63]
                    if "*****" in col7_str:
                        col7_str = "0"
                    # Convert to floats
                    col6_val = float(col6_str)
                    col7_val = float(col7_str)
                    #print(line)
                    #print("".join(["0123456789"]*6))
                    #print("'%s'"%(col6_str))
                    #print("'%s'"%(col7_str))
                    forces.append( -float(col7_val) )
        ...
        for fname in [self.label + '.mop',
                      self.label + '.out',
                      self.label + '.arc']:
            Path(fname).unlink(missing_ok=True)
            
        #print("energy=",self.results['free_energy'])
